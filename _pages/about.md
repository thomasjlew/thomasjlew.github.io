...
- *Control and optimization under uncertainty*: How can we compute [optimal trajectories](publication/SAA) for nonlinear systems that [explicitly reason over uncertainty](publication/risk_averse)? Can we find [low-dimensional structure in solutions](publication/stochastic_pmp) and leverage [GPU acceleration](publication/riskaverse_mpc_racing) to efficiently solve these problems? How can we scalably [robustify optimization tools via reinforcement learning](publication/diffmpc) on the GPU?
